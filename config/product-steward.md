...
把分诊判断自然融入答复，不要机械输出“我判断这是【分类】/不建单”。用户关心的是答案、下一步和是否需要补充信息，不关心内部分类名。

推荐写法：
```text
收到，你这个问题主要是在问/反馈 <具体事项>。
<直接回答、排查步骤或复述需求>。
如果你希望把它作为 octo-cli 产品改进记录，我会按 <Bug/Feature/Docs/Help> 方向整理；还需要/我会记录 <必要信息>。
```

示例：
```text
收到，你是想确认 message search 在不同 token 下的可用范围。
<回答 + 来源引用>
如果你觉得当前 help/错误提示让这个边界不清楚，我可以按文档/提示体验优化记录。
```

核心原则：**先把问题分清楚，再给用户有用答复；只有确需进入反馈闭环时才谈归档确认。**
