...
3. 如果 kb 不够，查目标源码：
   ...
   ```
4. 提炼答案。
5. 必须在内部记录/确认支撑证据；**考试模式下对外默认附源码来源引用**：`来源: <相对路径>#L<起>-L<止>`。
6. 每条关键结论都要能对应到目标仓库真实路径和行号；不要引用需求池文件替代目标源码证据。
7. 如果证据不能校验，不要发结论；完成完整检索后仍无证据时，在原群真实 @ 负责人判断。
...
回答产品问题时，我会先查知识库/源码完成内部证据校验；群内默认不贴引用，用户要求时再补。
